...
router.post('/stream', async (req: AuthRequest, res) => {
  const { title, content, provider = 'gemini', model } = req.body;
  const userId = req.user?.userId;

  console.log(`📥 POST /api/documents-stream/stream - Title: ${title}, Provider: ${provider}${model ? `, Model: ${model}` : ''}`);
  ...
  try {
    ...
    try {
      ...
      for await (const chunk of generateSummaryStream(content, provider, model)) {
        fullSummary += chunk;
    ...
    try {
      ...
      const flashcardData = await generateFlashcards(content, provider, model);
